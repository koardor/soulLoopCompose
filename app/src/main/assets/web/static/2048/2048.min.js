  ...
  listen() {
    ...
    document.addEventListener("keydown", (c) => {
      const d = c.altKey || c.ctrlKey || c.metaKey || c.shiftKey,
        e = b[c.which];
      d || e === void 0 || (c.preventDefault(), a.emit("move", e)),
    ...
    e.addEventListener(this.eventTouchstart, (a) => {
      ...
      e.addEventListener(this.eventTouchend, (b) => {
        if (
          (!window.navigator.msPointerEnabled && 0 < b.touches.length) ||
        ...
        const g = e - c,
          ...
          j = Math.abs(i);
        10 < Math.max(h, j) &&
          a.emit("move", h > j ? (0 < g ? 1 : 3) : 0 < i ? 2 : 0);
      });
  }
  restart(a) {
...
class HTMLActuator {
  ...
  addTile(a) {
    const b = this,
      ...
      e = a.previousPosition || { x: a.x, y: a.y },
  ...
  updateScore(a) {
    ...
    if (
      ((this.score = a), (this.scoreContainer.textContent = this.score), 0 < b)
    ) {
...
export class GameManager {
  ...
  move(a) {
    ...
    this.prepareTiles(),
      f.x.forEach((a) => {
        f.y.forEach((f) => {
          if (((c = { x: a, y: f }), (d = b.grid.cellContent(c)), d)) {
  ...
  buildTraversals({ x: a, y: b }) {
    ...
    return (
      1 === a && (c.x = c.x.reverse()), 1 === b && (c.y = c.y.reverse()), c
    );
  }
  findFarthestPosition(a, { x: b, y: c }) {
    let d;
    do (d = a), (a = { x: d.x + b, y: d.y + c });
  ...
  tileMatchesAvailable() {
    ...
    for (let c = 0; c < this.size; c += 1)
      for (let d = 0; d < this.size; d += 1)
        if (((b = this.grid.cellContent({ x: c, y: d })), b))
          for (let e = 0; 4 > e; e += 1) {
